...
app.whenReady().then(async () => {
  ...
  ipcMain.handle('rag:processFile', async (_, filePath) => {
    try {
      ...
      return { success: true, count: docs.length, preview: docs[0]?.pageContent.slice(0, 100) }
    } catch (error) {
  ...
  ipcMain.on('rag:chat', async (event, question) => {
    try {
      console.log('Chat question:', question)
      const { stream, sources } = await chatWithRag(question)
