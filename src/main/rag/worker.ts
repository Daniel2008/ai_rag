    ...
    if (type === 'initEmbedding') {
      ...
      try {
        ...
        progressCheckInterval = setInterval(() => {
          ...
          console.log(`Progress check: ${elapsedTime}ms since start, last progress: ${lastReportedGlobalProgress}`)
        ...
        // 发送进度更新的函数
        const sendProgressUpdate = (status: ProgressStatus, message: string, file?: string, fileProgress?: number) => {
          // 节流控制，避免闪烁
          const now = Date.now();
          ...
          const progressMessage: ProgressMessage = {
            ...
            payload: {
              ...
              message,
              file,
              fileProgress: fileProgress !== undefined ? Math.max(0, Math.min(1, fileProgress)) : undefined
            }
          };
        ...
        console.log('[DEBUG] Completed files:', Array.from(fileStates.values()).filter(s => s.completed).length)
        
        // 等待一小段时间让文件下载完成
        await new Promise(resolve => setTimeout(resolve, 1000))
        
        // 最终检查文件状态
        const finalCompletedFiles = Array.from(fileStates.values()).filter(s => s.completed).length
        const finalTotalFiles = fileStates.size
        
        console.log('[DEBUG] Final file status - Completed:', finalCompletedFiles, 'Total:', finalTotalFiles)
